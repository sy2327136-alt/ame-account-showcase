(function () {
  ...
  function card(product) {
    ...
    return `<article class="product-card home-product-card">
      ...
      <a href="./${product.productType === "account" ? "accounts" : "recharge"}.html?game=${encodeURIComponent(product.gameName)}" class="card-image"><img src="${GM.escape(cover)}" alt="${GM.escape(product.title)}" loading="lazy" referrerpolicy="no-referrer">${product.badge ? `<span class="card-badge">${GM.escape(product.badge)}</span>` : ""}${images.length ? `<span class="photo-count">▧ ${images.length}图</span>` : '<span class="photo-count">暂无实拍</span>'}</a>
